...
$redirect = isset($_POST['redirect']) ? $_POST['redirect'] : qf_url_page('index.php');
...
if (!qf_can_delete_attachment($att, $u)) {
    header('Content-Type: text/html; charset=utf-8', true, 403);
    exit('你没有权限删除这个附件。');
}

$file_deleted = qf_delete_attachment_file($att['file_path']);
if (!$file_deleted) {
...
while ($post_rows && ($row = mysqli_fetch_assoc($post_rows))) {
    ...
    $content_sql = esc($new_content);
    $row_id = intval($row['id']);
    mysqli_query(db(), "UPDATE qf_posts SET content='{$content_sql}' WHERE id={$row_id}");
}

mysqli_query(db(), "DELETE FROM qf_attachments WHERE id={$tag_id}");
