...
$posts = mysqli_query(db(), "SELECT p.*, t.title, t.id AS thread_id, t.is_good FROM qf_posts p LEFT JOIN qf_threads t ON t.id=p.thread_id WHERE p.user_id=" . intval($user['id']) . " AND p.is_deleted=0 AND t.is_deleted=0 ORDER BY p.created_at DESC LIMIT 20");
?>
...
<section class="card thread-list phpdo-user-replies">
    ...
    <?php $count = 0; while ($posts && ($p = mysqli_fetch_assoc($posts))) { $count++; ?>
        <div class="thread-row">
            <span class="phpdo-avatar" aria-hidden="true"><img src="<?php echo h($avatar); ?>" alt=""></span>
            <div class="thread-main">
                <a class="thread-title<?php echo intval($p['is_good']) ? ' phpdo-title-good' : ''; ?>" href="<?php echo h(qf_url_thread($p['thread_id'])); ?>#replies"><?php echo h($p['title']); ?></a>
