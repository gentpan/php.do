...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['ignore_all']) && qf_notifications_ready()) {
    ...
    if (isset($_POST['notification_sound_enabled'])) {
        ...
        mysqli_query(db(), "UPDATE qf_users SET notification_sound_enabled={$sound_enabled} WHERE id={$uid}");
        $u['notification_sound_enabled'] = $sound_enabled;
        $notice = $sound_enabled ? '已开启语音提示。' : '已关闭语音提示。';
    }
}
$items = qf_notifications_ready() ? mysqli_query(db(), "SELECT * FROM qf_notifications WHERE user_id={$uid} ORDER BY id DESC LIMIT 50") : false;
if (qf_notifications_ready()) {
    mysqli_query(db(), "UPDATE qf_notifications SET is_read=1 WHERE user_id={$uid}");
}
...
<section class="card">
    ...
    <div class="notification-actions">
        ...
        <form method="post">
            <?php if (qf_notification_sound_enabled($u)) { ?>
