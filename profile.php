    ...
    if ($error === '') {
        ...
        if ($password !== '') {
            if ($password !== $password_confirm) {
                ...
                mysqli_query(db(), "UPDATE qf_users SET nickname='{$nickname_sql}', avatar='{$avatar_sql}', signature='{$signature_sql}', gender='{$gender_sql}', notification_sound_enabled={$notification_sound_enabled}, password='{$password_sql}' WHERE id=" . intval($u['id']));
                $saved = true;
            }
        } else {
            mysqli_query(db(), "UPDATE qf_users SET nickname='{$nickname_sql}', avatar='{$avatar_sql}', signature='{$signature_sql}', gender='{$gender_sql}', notification_sound_enabled={$notification_sound_enabled} WHERE id=" . intval($u['id']));
            $saved = true;
        }
        $u = current_user();
    }
